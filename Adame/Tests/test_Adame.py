...
Tests that the start-command works as expected"""
        ...
        try:
            ...
            environment_for_test.adame._internal_sc.register_mock_program_call("docker-compose",  re.escape(
                f"--project-name {environment_for_test.adame._internal_get_container_name()} up --detach --build --quiet-pull --remove-orphans --force-recreate --always-recreate-deps"), re.escape(environment_for_test.adame.__internal_configuration_folder), 0, "", "", 40)
            environment_for_test.adame._internal_sc.register_mock_program_call("snort", re.escape(
                f'-D -i eth0 -c "{environment_for_test.adame.__internal_networktrafficgeneratedrules_file}" -l "{environment_for_test.adame.__internal_log_folder_for_ids}" -U -v -x -y -K ascii'), "", 0, "", "", 44)
...
Tests that the stop-command works as expected"""
        ...
        try:
            ...
            # mock program calls which are maybe not available in a development-environment:
            environment_for_test.adame._internal_sc.register_mock_program_call("docker-compose",  re.escape(
                f"--project-name {environment_for_test.adame._internal_get_container_name()} down --remove-orphans"), re.escape(environment_for_test.adame.__internal_configuration_folder), 0, "", "", 68)
            environment_for_test.adame.register_mock_process_query(
                44, f'snort -D -i eth0 -c "{environment_for_test.adame.__internal_networktrafficgeneratedrules_file}" -l "{environment_for_test.adame.__internal_log_folder_for_ids}" -U -v -x -y -K ascii')
